...
export function ResultsStep({ result, isLoading, onRetry }: ResultsStepProps) {
  ...
  const handleShare = useCallback(async () => {
    ...
    setSharing(true)
    try {
      const res = await fetch(`/api/reports/${result.analysisId}/share`, {
        method: "POST",
      })
      ...
      <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-border bg-surface p-16">
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-secondary">
            Extracting fees, rates, and contract terms
          </p>
        </div>
      </div>
    )
  ...
  if (!result) {
    return (
      <EmptyState
        ...
        description="Upload and submit an agreement to see your analysis."
        action={
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <PrimaryButton variant="outline" size="sm" onClick={onRetry}>
            <RotateCcw className="h-4 w-4" />
            New Analysis
          </PrimaryButton>
          {result.analysisId && (
            <PrimaryButton
              size="sm"
              onClick={() => router.push(`/reports/${result.analysisId}`)}
            >
              View Report
            </PrimaryButton>
          )}
